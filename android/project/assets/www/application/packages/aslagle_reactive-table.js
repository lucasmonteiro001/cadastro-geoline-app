...
i18n.map('fa', {                                                                                                     // 234
    reactiveTable: {                                                                                                 // 235
        ...
        columns: 'ستون',                                                                                             // 237
        show: 'ارائه',                                                                                               // 238
        rowsPerPage: 'ردیف در هر صفحه',                                                                              // 239
        page: 'صفحه',                                                                                                // 240
        of: 'از'                                                                                                     // 241
    }                                                                                                                // 242
});                                                                                                                  // 243
...
i18n.map('gr', {                                                                                                     // 245
    reactiveTable: {                                                                                                 // 246
        ...
        columns: 'Στήλες',                                                                                           // 248
        show: 'Προβολή',                                                                                             // 249
        rowsPerPage: 'γραμμές ανά σελίδα',                                                                           // 250
        page: 'Σελίδα',                                                                                              // 251
        of: 'από'                                                                                                    // 252
    }                                                                                                                // 253
});                                                                                                                  // 254
...
var getFieldMatches = function (field) {                                                                             // 36
  ...
  var extraMatch = field.replace(/\.\d+\./g, ".");                                                                   // 44
  if (fieldMatches.indexOf(extraMatch) === -1) fieldMatches.push(extraMatch);                                        // 45
  return fieldMatches;                                                                                               // 46
};                                                                                                                   // 47
                                                                                                                     // 48
getFilterQuery = function (filterInputs, filterFields, settings) {                                                   // 49
  settings = settings || {};                                                                                         // 50
  if (settings.enableRegex === undefined) {                                                                          // 51
    settings.enableRegex = false;                                                                                    // 52
  }                                                                                                                  // 53
  if (settings.filterOperator === undefined) {                                                                       // 54
    settings.filterOperator = "$and";                                                                                // 55
  }                                                                                                                  // 56
  if (settings.fields) {                                                                                             // 57
    _.each(filterInputs, function (filter, index) {                                                                  // 58
      if (_.any(settings.fields, function (include) { return include; })) {                                          // 59
        filterFields[index] = _.filter(filterFields[index], function (field) {                                       // 60
          return _.any(getFieldMatches(field), function (fieldMatch) {                                               // 61
            return settings.fields[fieldMatch];                                                                      // 62
          });                                                                                                        // 63
        });                                                                                                          // 64
      } else {                                                                                                       // 65
        filterFields[index] = _.filter(filterFields[index], function (field) {                                       // 66
          return _.all(getFieldMatches(field), function (fieldMatch) {                                               // 67
            return _.isUndefined(settings.fields[fieldMatch]) || settings.fields[fieldMatch];                        // 68
          });                                                                                                        // 69
        });                                                                                                          // 70
      }                                                                                                              // 71
    });                                                                                                              // 72
  }                                                                                                                  // 73
  var numberRegExp = /^\d+$/;                                                                                        // 74
  var queryList = [];                                                                                                // 75
  _.each(filterInputs, function (filter, index) {                                                                    // 76
    if (filter) {                                                                                                    // 77
      if (_.isObject(filter)) {                                                                                      // 78
        var fieldQueries = _.map(filterFields[index], function (field) {                                             // 79
          var query = {};                                                                                            // 80
          query[field] = filter;                                                                                     // 81
          return query;                                                                                              // 82
        });                                                                                                          // 83
        if (fieldQueries.length) {                                                                                   // 84
            queryList.push({'$or': fieldQueries});                                                                   // 85
          }                                                                                                          // 86
      } else {                                                                                                       // 87
        var filters = parseFilterString(filter);                                                                     // 88
        _.each(filters, function (filterWord) {                                                                      // 89
          if (settings.enableRegex === false) {                                                                      // 90
            filterWord = escapeRegex(filterWord);                                                                    // 91
          }                                                                                                          // 92
          var filterQueryList = [];                                                                                  // 93
          _.each(filterFields[index], function (field) {                                                             // 94
            var filterRegExp = new RegExp(filterWord, 'i');                                                          // 95
            var query = {};                                                                                          // 96
            query[field] = filterRegExp;                                                                             // 97
            filterQueryList.push(query);                                                                             // 98
                                                                                                                     // 99
            if (numberRegExp.test(filterWord)) {                                                                     // 100
              var numberQuery = {};                                                                                  // 101
              numberQuery[field] = parseInt(filterWord, 10);                                                         // 102
              filterQueryList.push(numberQuery);                                                                     // 103
            }                                                                                                        // 104
                                                                                                                     // 105
            if (filterWord === "true") {                                                                             // 106
              var boolQuery = {};                                                                                    // 107
              boolQuery[field] = true;                                                                               // 108
              filterQueryList.push(boolQuery);                                                                       // 109
            } else if (filterWord === "false") {                                                                     // 110
              var boolQuery = {};                                                                                    // 111
              boolQuery[field] = false;                                                                              // 112
              filterQueryList.push(boolQuery);                                                                       // 113
            }                                                                                                        // 114
          });                                                                                                        // 115
                                                                                                                     // 116
          if (filterQueryList.length) {                                                                              // 117
            var filterQuery = {'$or': filterQueryList};                                                              // 118
            queryList.push(filterQuery);                                                                             // 119
          }                                                                                                          // 120
        });                                                                                                          // 121
      }                                                                                                              // 122
    }                                                                                                                // 123
  ...
  var query = {};                                                                                                    // 126
                                                                                                                     // 127
  if(queryList.length) {                                                                                             // 128
    query[settings.filterOperator] = queryList;                                                                      // 129
  }                                                                                                                  // 130
                                                                                                                     // 131
  return query;                                                                                                      // 132
};                                                                                                                   // 133
...
if (Meteor.isClient) {                                                                                               // 135
  ReactiveTable = ReactiveTable || {};                                                                               // 136
                                                                                                                     // 137
  var reactiveTableFilters = {};                                                                                     // 138
  var callbacks = {};                                                                                                // 139
                                                                                                                     // 140
  ReactiveTable.Filter = function (id, fields) {                                                                     // 141
    if (reactiveTableFilters[id]) {                                                                                  // 142
      reactiveTableFilters[id].fields = fields;                                                                      // 143
      return reactiveTableFilters[id];                                                                               // 144
    }                                                                                                                // 145
                                                                                                                     // 146
    var filter = new ReactiveVar();                                                                                  // 147
                                                                                                                     // 148
    this.fields = fields;                                                                                            // 149
                                                                                                                     // 150
    this.get = function () {                                                                                         // 151
      return filter.get() || '';                                                                                     // 152
    };                                                                                                               // 153
                                                                                                                     // 154
    this.set = function (filterString) {                                                                             // 155
      filter.set(filterString);                                                                                      // 156
      _.each(callbacks[id], function (callback) {                                                                    // 157
        callback();                                                                                                  // 158
      });                                                                                                            // 159
    };                                                                                                               // 160
                                                                                                                     // 161
    reactiveTableFilters[id] = this;                                                                                 // 162
  };                                                                                                                 // 163
                                                                                                                     // 164
  ReactiveTable.clearFilters = function (filterIds) {                                                                // 165
    _.each(filterIds, function (filterId) {                                                                          // 166
      if (reactiveTableFilters[filterId]) {                                                                          // 167
        reactiveTableFilters[filterId].set('');                                                                      // 168
      }                                                                                                              // 169
    });                                                                                                              // 170
  };                                                                                                                 // 171
                                                                                                                     // 172
  dependOnFilters = function (filterIds, callback) {                                                                 // 173
    _.each(filterIds, function (filterId) {                                                                          // 174
      if (_.isUndefined(callbacks[filterId])) {                                                                      // 175
        callbacks[filterId] = [];                                                                                    // 176
      }                                                                                                              // 177
      callbacks[filterId].push(callback);                                                                            // 178
    });                                                                                                              // 179
  };                                                                                                                 // 180
                                                                                                                     // 181
  getFilterStrings = function (filterIds) {                                                                          // 182
    return _.map(filterIds, function (filterId) {                                                                    // 183
      if (_.isUndefined(reactiveTableFilters[filterId])) {                                                           // 184
        reactiveTableFilters[filterId] = new ReactiveTable.Filter(filterId);                                         // 185
      }                                                                                                              // 186
      return reactiveTableFilters[filterId].get();                                                                   // 187
    });                                                                                                              // 188
  };                                                                                                                 // 189
                                                                                                                     // 190
  getFilterFields = function (filterIds, allFields) {                                                                // 191
    return _.map(filterIds, function (filterId) {                                                                    // 192
      if (_.isUndefined(reactiveTableFilters[filterId])) {                                                           // 193
        return _.map(allFields, function (field) { return field.key; });                                             // 194
      } else if (_.isEmpty(reactiveTableFilters[filterId].fields)) {                                                 // 195
        return _.map(allFields, function (field) { return field.key; });                                             // 196
      } else {                                                                                                       // 197
        return reactiveTableFilters[filterId].fields;                                                                // 198
      }                                                                                                              // 199
    });                                                                                                              // 200
  };                                                                                                                 // 201
                                                                                                                     // 202
  Template.reactiveTableFilter.helpers({                                                                             // 203
    'class': function () {                                                                                           // 204
      return this.class || 'input-group';                                                                            // 205
    },                                                                                                               // 206
                                                                                                                     // 207
    'filter': function () {                                                                                          // 208
      if (_.isUndefined(reactiveTableFilters[this.id])) {                                                            // 209
        new ReactiveTable.Filter(this.id, this.fields);                                                              // 210
      } else if (_.isUndefined(reactiveTableFilters[this.id].fields)) {                                              // 211
        reactiveTableFilters[this.id].fields = this.fields;                                                          // 212
      }                                                                                                              // 213
      return reactiveTableFilters[this.id].get();                                                                    // 214
    }                                                                                                                // 215
  });                                                                                                                // 216
                                                                                                                     // 217
  var updateFilter = _.debounce(function (template, filterText) {                                                    // 218
    reactiveTableFilters[template.data.id].set(filterText);                                                          // 219
  }, 200);                                                                                                           // 220
                                                                                                                     // 221
  Template.reactiveTableFilter.events({                                                                              // 222
    'keyup .reactive-table-input, input .reactive-table-input': function (event) {                                   // 223
      var template = Template.instance();                                                                            // 224
      var filterText = $(event.target).val();                                                                        // 225
      updateFilter(template, filterText);                                                                            // 226
    },                                                                                                               // 227
  });                                                                                                                // 228
}                                                                                                                    // 229
                                                                                                                     // 230
